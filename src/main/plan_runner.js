...
// Extract JSON plan from LLM output, handling markdown fences and extra text
function extractPlanJson(outputBuffer) {
  // Try <plan_json> tags first
  const tagMatch = outputBuffer.match(/<plan_json>([\s\S]*?)<\/plan_json>/i)
...
// Update prd.json items with plan details keyed by id
async function updatePrdWithPlans(folderPath, plans) {
  try {
    ...
    for (const item of prdContent) {
      if (plans[item.id] !== undefined) {
        item.plan = plans[item.id]
        updated++
      }
    }
...
export async function executePlan(folderPath) {
  try {
    ...
    child.on('close', async (code, signal) => {
      ...
      if (code === 0 || outputBuffer.includes('</plan_json>')) {
